...
class Attention4D(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        q = (
            self.q(x)
            .flatten(2)
            .reshape(B, self.num_heads, -1, self.N)
            .permute(0, 1, 3, 2)
        )
        k = (
            self.k(x)
        ...
        out = (
            x.transpose(2, 3).reshape(B, self.dh, self.resolution, self.resolution)
            + v_local
        )
...
class Attention4DDownsample(torch.nn.Module):
    def __init__(
        ...
        self.q = LGQuery(
            dim, self.num_heads * self.key_dim, self.resolution, self.resolution2
        )
        ...
        for p1 in points_:
            for p2 in points:
                ...
                offset = (
                    abs(
                        p1[0] * math.ceil(self.resolution / self.resolution2)
                        - p2[0]
                        + (size - 1) / 2
                    ),
                    abs(
                        p1[1] * math.ceil(self.resolution / self.resolution2)
                        - p2[1]
                        + (size - 1) / 2
                    ),
                )
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        q = (
            self.q(x)
            .flatten(2)
            .reshape(B, self.num_heads, -1, self.N2)
            .permute(0, 1, 3, 2)
        )
        k = (
            self.k(x)
